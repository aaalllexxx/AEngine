...
def create_cluster_node(node_id: str, role: str, master_ip: str, master_port: int,
                        sync_dir: str = ".", heartbeat_interval: int = 2,
                        timeout: int = 6) -> ClusterNode:
    """Фабрика узла кластера Active-Passive (см. ClusterNode)."""
    return ClusterNode(
        node_id=node_id, role=role, master_ip=master_ip, master_port=master_port,
        sync_dir=sync_dir, heartbeat_interval=heartbeat_interval, timeout=timeout,
    )


__all__ = ['ClusterNode', 'create_cluster_node']
