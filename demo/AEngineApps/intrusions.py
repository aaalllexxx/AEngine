...
class RuleDetector(BaseDetector):
    ...
    Каждое правило — функция condition(request) -> bool. Если возвращает True,
    запрос считается атакой. Правила хранятся на уровне ЭКЗЕМПЛЯРА (а не класса),
    чтобы детекторы разных приложений не делили один список.
    """

    def __init__(self, app):
        super().__init__(app)
        self.rules = []  # список (condition_fn, action_msg)
    ...
    def run(self) -> None:
        for condition, msg in self.rules:
            try:
                ...
                # Не глушим молча: кривое правило не должно ронять IDS, но должно быть видно в логах.
                self.app.logger.warning(f"[RuleDetector] Ошибка в правиле '{msg}': {e}")
...
class IDS:
    ...
    def __init__(self, app, is_enabled: Optional[Callable[[], bool]] = None):
        self.app: Flask = app.flask
        self.detectors: list[BaseDetector] = []
        self.detect_funcs: list[Callable] = []
        # Предикат «модуль включён» — позволяет включать/выключать защиту на лету
        # (например, из дашборда), не перерегистрируя хук.
        self._is_enabled = is_enabled or (lambda: True)
        self.app.before_request(self.run_detectors)

    def add_detector(self, detector_cls: type) -> "BaseDetector":
        """Добавляет детектор и возвращает его экземпляр."""
        detector = detector_cls(self.app)
        self.detectors.append(detector)
        return detector

    def get_detector(self, detector_cls: type) -> Optional["BaseDetector"]:
        """Возвращает уже добавленный детектор указанного типа (или None)."""
        for detector in self.detectors:
            if isinstance(detector, detector_cls):
                return detector
        return None

    def add_rule(self, condition_fn: Callable, action_msg: str = "Rule Triggered") -> None:
        """Добавляет пользовательское правило в RuleDetector (создаёт его при необходимости)."""
        rule_detector = self.get_detector(RuleDetector)
        if rule_detector is None:
            rule_detector = self.add_detector(RuleDetector)
        rule_detector.add_rule(condition_fn, action_msg)

    def run_detectors(self) -> None:
        """Запускает все детекторы на текущем запросе (если модуль включён)."""
...
class IPS(IDS):
    ...
    def __init__(self, app, is_enabled: Optional[Callable[[], bool]] = None):
        super().__init__(app, is_enabled=is_enabled)
